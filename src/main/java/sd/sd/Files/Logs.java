...
public class Logs {
    ...
    public void addLog(String importance, String log){

        List<String> list = newConfig.getStringList("Logs");

        list.add(importance.replace("<prefix>", SD.prefix).replace("&", "§").replace("<discord>", SD.getInstance().getConfig().getString("info.discord", "") + log.replace("<prefix>", SD.prefix).replace("&", "§").replace("<discord>", SD.getInstance().getConfig().getString("info.discord", ""))));

        newConfig.set("Logs", list);
    }
}
